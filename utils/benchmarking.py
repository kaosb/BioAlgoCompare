...
class BenchmarkResult:
    ...
    def compute_metrics(self):
        ...
        if self.optimal_value:
            self.gap_to_optimal = (self.best_fitness - self.optimal_value) / self.optimal_value * 100
            
            # Tasa de éxito (soluciones dentro del 1% del óptimo)
            threshold = self.optimal_value * 1.01
            successful_runs = sum(1 for fitness in self.fitness_values if fitness <= threshold)
        ...
        if self.convergence_curves:
            ...
            standardized_curves = [curve[:min_length] for curve in self.convergence_curves]
...
def run_benchmark(algorithms, problem_instances, runs=10, iterations=100, population=30, seed=None, parallel=False):
    ...
    if parallel:
        pool = mp.Pool(processes=min(mp.cpu_count(), len(algorithms) * len(problem_instances)))
        tasks = []
        ...
        for algo_name, AlgoClass in algorithms.items():
            ...
            if parallel:
                ...
                for run in range(runs):
                    ...
                    tasks.append((AlgoClass, problem, population, iterations, run_seed, benchmark_result))
            else:
                ...
                for run in range(runs):
                    ...
                    print(f"    Ejecución {run+1}/{runs}: Fitness = {best_solution.fitness():.2f}, Tiempo = {execution_time:.2f}s")
                
                benchmark_result.compute_metrics()
                results.append(benchmark_result)
                
                print(f"  Mejor: {benchmark_result.best_fitness:.2f}, Promedio: {benchmark_result.mean_fitness:.2f}, " +
                      f"Tiempo: {benchmark_result.mean_time:.2f}s")
                if benchmark_result.optimal_value:
                    print(f"  Gap al óptimo: {benchmark_result.gap_to_optimal:.2f}%, " +
    ...
    for i, (instance_name, results) in enumerate(instances.items()):
        ...
        for result in results:
            if result.avg_convergence is not None:
                ...
                ax.plot(iterations, result.avg_convergence, linewidth=2, label=result.algorithm_name)
...
def create_benchmark_report(benchmark_results, filename=None):
    ...
    # Organizar resultados por instancia
    instances = {}
    for result in benchmark_results:
        if result.instance_name not in instances:
            instances[result.instance_name] = []
    ...
    for instance_name, results in instances.items():
        for result in results:
            summary_data.append({
                ...
                'Gap (%)': f"{result.gap_to_optimal:.2f}" if result.gap_to_optimal is not None else "N/A",
                'Success (%)': f"{result.success_rate:.2f}" if result.success_rate is not None else "N/A"
            })
    
    df_summary = pd.DataFrame(summary_data)
    
    # Crear CSS para el informe
    css = """
    <style>
        body {
    ...
    # Generar y guardar gráficos para cada instancia
    ...
    for instance_name, results in instances.items():
        ...
        # Gráficos específicos para esta instancia
        instance_results = [r for r in benchmark_results if r.instance_name == instance_name]
        
        # 1. Calidad de solución
        plt_quality = plot_solution_quality(instance_results)
        quality_path = os.path.join(figures_dir, f"{instance_name}_quality.png")
        plt_quality.savefig(quality_path)
        plt_quality.close()
        
        html += f"""
            <div class="figure">
                <img src="figures/{instance_name}_quality.png" alt="Solution Quality">
                <div class="caption">Figure: Solution quality comparison for {instance_name}</div>
            </div>
        """
        
        # 2. Tiempo de ejecución
        plt_time = plot_execution_time(instance_results)
        time_path = os.path.join(figures_dir, f"{instance_name}_time.png")
        plt_time.savefig(time_path)
        plt_time.close()
        
        html += f"""
            <div class="figure">
                <img src="figures/{instance_name}_time.png" alt="Execution Time">
                <div class="caption">Figure: Execution time comparison for {instance_name}</div>
            </div>
        """
        
        # 3. Convergencia
        plt_conv = plot_convergence_comparison(instance_results)
        conv_path = os.path.join(figures_dir, f"{instance_name}_convergence.png")
        plt_conv.savefig(conv_path)
        plt_conv.close()
        
        html += f"""
            <div class="figure">
                <img src="figures/{instance_name}_convergence.png" alt="Convergence Curves">
                <div class="caption">Figure: Convergence curve comparison for {instance_name}</div>
            </div>
        """
        
        # 4. Radar de rendimiento
        plt_radar = plot_performance_radar(instance_results, instance_name)
        if plt_radar:
            radar_path = os.path.join(figures_dir, f"{instance_name}_radar.png")
            plt_radar.savefig(radar_path)
            plt_radar.close()
            
            html += f"""
                <div class="figure">
                    <img src="figures/{instance_name}_radar.png" alt="Performance Radar">
                    <div class="caption">Figure: Performance radar chart for {instance_name}</div>
                </div>
            """
        
        html += "</div>"  # Cierra la sección de la instancia
    ...
    if len(benchmark_results) > 0:
        ...
        # Agrupar por instancia para análisis estadístico
        for instance_name, results in instances.items():
            if len(results) >= 2:  # Necesitamos al menos 2 algoritmos para comparar
                ...
                # Asegurar que todas las muestras tengan el mismo tamaño
                min_samples = min(len(s) for s in samples)
                samples = [s[:min_samples] for s in samples]
                
                # Realizar el test de Friedman si hay suficientes muestras
                if min_samples >= 5 and len(samples) >= 2:
                    try:
                        # Convertir a format adecuado para friedmanchisquare
                        ...
                        # Si hay diferencias significativas, realizar tests post-hoc
                                ...
                                for j in range(i+1, len(samples)):
                                    try:
                                        ...
                                        <tr>
                                            <td>{algorithm_names[i]}</td>
                                            <td>{algorithm_names[j]}</td>
                                            <td colspan="2">Error: {str(e)}</td>
                                        </tr>
                                        """
                            
                            html += "</table>"
                    except Exception as e:
                        html += f"<p>Error during statistical analysis: {str(e)}</p>"
                else:
                    html += "<p>Insufficient data for statistical analysis. Need at least 5 runs per algorithm and 2 algorithms.</p>"
            else:
                html += f"<p>Only one algorithm available for {instance_name}, statistical comparison not possible.</p>"
        
        html += "</div>"  # Cierra la sección de análisis estadístico
...
if __name__ == "__main__":
    ...
    # Definir instancias para el benchmark
    instances = ["E-n22-k4", "P-n16-k8"]
    
    # Ejecutar benchmark
    results = run_benchmark(algorithms, instances, runs=5, iterations=100, population=30, seed=42)
    
    # Guardar resultados
    save_benchmark_results(results)
    
    # Crear visualizaciones
    plot_solution_quality(results, "Comparación de Calidad de Solución").savefig("results/quality_comparison.png")
    plot_execution_time(results, "Comparación de Tiempo de Ejecución").savefig("results/time_comparison.png")
    plot_convergence_comparison(results, "Comparación de Convergencia").savefig("results/convergence_comparison.png")
