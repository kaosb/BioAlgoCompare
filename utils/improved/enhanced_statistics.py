...
class EnhancedStatisticalAnalysis:
    ...
    def prepare_data_for_statistics(
        benchmark_results, metric="best_fitness", include_opt=True
    ):
        """
    ...
    def prepare_raw_data_for_statistics(benchmark_results, include_opt=True):
        ...
        for result in benchmark_results:
            ...
            for run_id, (fitness, time) in enumerate(
                zip(result.fitness_values, result.execution_times), 1
            ):
                data_rows.append(
                    {
                        "algorithm": result.algorithm_name,
    ...
    def perform_friedman_test(data, alpha=0.05):
        ...
        if pivot.shape[0] < 2 or pivot.shape[1] < 2:
            logger.warning(
                "Se requieren al menos 2 algoritmos y 2 instancias para el test de Friedman"
            )
            return {
                "test": "Friedman",
                "reject_h0": False,
        ...
        critical_distance = stats.t.ppf(1 - alpha / 2, n - 1) * np.sqrt(
            (k * (k + 1)) / (6 * n)
        )
    ...
    def perform_nemenyi_test(friedman_result):
        ...
        if "rank_dict" not in friedman_result or not friedman_result["rank_dict"]:
            logger.warning(
                "No hay resultados de Friedman para realizar test de Nemenyi"
            )
            return {"test": "Nemenyi", "error": "No hay resultados de Friedman"}
    ...
    def perform_wilcoxon_test(data, alpha=0.05):
        ...
        if len(algorithms) < 2:
            logger.warning(
                "Se requieren al menos 2 algoritmos para el test de Wilcoxon"
            )
            return pd.DataFrame()
        ...
        for instance in instances:
            ...
            for algo1, algo2 in combinations(algorithms, 2):
                algo1_fitness = inst_data[inst_data["algorithm"] == algo1][
                    "fitness"
                ].values
                algo2_fitness = inst_data[inst_data["algorithm"] == algo2][
                    "fitness"
                ].values
                ...
                if len(algo1_fitness) < 5 or len(algo2_fitness) < 5:
                    logger.warning(
                        f"Datos insuficientes para {algo1} vs {algo2} en {instance}"
                    )
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        f"Error en test de Wilcoxon para {algo1} vs {algo2} en {instance}: {str(e)}"
                    )
    ...
    def perform_bootstrap_analysis(
        data, metric="fitness", n_bootstrap=1000, confidence=0.95
    ):
        """
        ...
        for (algo, instance), group in grouped:
            ...
            if len(values) < 10:
                logger.warning(
                    f"Datos insuficientes para bootstrap: {algo}, {instance}"
                )
                continue
            ...
            try:
                ...
                if p_value < alpha:
                    ...
                    all_labels = np.concatenate(
                        [[algo] * len(s) for algo, s in zip(sample_names, samples)]
                    )
                    ...
                    for i, (algo1, algo2) in enumerate(result.data[0]):
                        ...
                        diff = -result.meandiffs[
                            i
                        ]  # Negativo porque queremos minimizar
                        conf_lower = -result.confint[i][
                            1
                        ]  # Negativo y cambiado porque queremos minimizar
                        conf_upper = -result.confint[i][
                            0
                        ]  # Negativo y cambiado porque queremos minimizar
                ...
                logger.warning(
                    f"Error en test de Kruskal-Wallis para {instance}: {str(e)}"
                )
                ...
                result_rows.append(
                    {
                        ...
                        "Effect_Size": abs(
                            d
                        ),  # Valor absoluto para facilitar interpretación
    ...
    def run_comprehensive_statistical_analysis(
        benchmark_results, metrics=None, output_dir=None, alpha=0.05
    ):
        """
        ...
        raw_data = EnhancedStatisticalAnalysis.prepare_raw_data_for_statistics(
            benchmark_results
        )
        ...
        for metric in metrics:
            ...
            data = EnhancedStatisticalAnalysis.prepare_data_for_statistics(
            ...
            try:
                ...
                friedman_result = EnhancedStatisticalAnalysis.perform_friedman_test(
                    data, alpha=alpha
                )
                ...
                with open(friedman_file, "w") as f:
                    ...
                    json.dump(
                        ...
                        f,
                        indent=2,
                        default=lambda x: float(x) if isinstance(x, np.float64) else x,
                    )

                # Si Friedman es significativo, realizar Nemenyi
                if friedman_result["reject_h0"]:
                    logger.info(
                        f"Test de Friedman significativo. Ejecutando Nemenyi para {metric}"
                    )
                    nemenyi_result = EnhancedStatisticalAnalysis.perform_nemenyi_test(
                        friedman_result
                    )
                    ...
                    with open(nemenyi_file, "w") as f:
                        ...
                        if "significant_diff" in nemenyi_result_json:
                            nemenyi_result_json[
                                "significant_diff"
                            ] = nemenyi_result_json["significant_diff"].tolist()
                        if "rank_diff" in nemenyi_result_json:
                            nemenyi_result_json["rank_diff"] = nemenyi_result_json[
                                "rank_diff"
                            ].tolist()
            ...
            try:
                if metric == "best_fitness" or metric == "mean_fitness":
                    ...
                    wilcoxon_results = (
                        EnhancedStatisticalAnalysis.perform_wilcoxon_test(
            ...
            try:
                if metric == "best_fitness" or metric == "mean_fitness":
                    ...
                    bootstrap_results = (
                        EnhancedStatisticalAnalysis.perform_bootstrap_analysis(
                            raw_data,
                            metric="fitness",
                            n_bootstrap=1000,
                            confidence=0.95,
                    ...
                    if not bootstrap_results.empty:
                        bootstrap_file = os.path.join(
                            metric_dir, "bootstrap_analysis.csv"
                        )
                        bootstrap_results.to_csv(bootstrap_file, index=False)
            except Exception as e:
                logger.warning(
                    f"Error en análisis de bootstrap para {metric}: {str(e)}"
                )
            ...
            try:
                if metric == "best_fitness" or metric == "mean_fitness":
                    ...
                    effect_sizes = EnhancedStatisticalAnalysis.calculate_effect_sizes(
                        raw_data, metric="fitness"
                    )
                ...
                logger.warning(
                    f"Error al calcular tamaños de efecto para {metric}: {str(e)}"
                )
            ...
            try:
                ...
                report_paths[
                    metric
                ] = EnhancedStatisticalAnalysis.generate_statistical_analysis_report(
