...
class EnhancedBenchmarkResult(BenchmarkResult):
    ...
    def from_dict(cls, data):
        ...
        result.timestamp = data.get("timestamp", datetime.now().strftime("%Y%m%d_%H%M%S"))
...
def _run_algorithm_task(args):
    ...
    algo_class, instance_name, instance_path, run_id, iterations, population, seed, checkpoint_dir = args
    ...
    try:
        ...
        algorithm = algo_class(problem, population_size=population, max_iterations=iterations, seed=run_seed)
        ...
        if checkpoint_dir:
            checkpoint_file = os.path.join(
                checkpoint_dir, 
                f"{algo_class.__name__}_{instance_name}_run{run_id}.pkl"
            )
        ...
        logger.error(f"Error en ejecución {run_id} de {algo_class.__name__} para {instance_name}: {str(e)}")
        return {
...
def run_massive_benchmark(
    ...
    benchmark_id = generate_benchmark_id(algorithm_classes, instances, seed, iterations, population)
    ...
    if resume and os.path.exists(checkpoint_file):
        try:
            ...
            logger.info(f"Reanudado con éxito: {len(benchmark_results)} resultados, {len(completed_tasks)} tareas completadas")
        except Exception as e:
    ...
    for algo_name, algo_class in algorithm_classes.items():
        for instance_name, instance_path in valid_instances:
            ...
            if existing_result is None:
                ...
                logger.info(f"Continuando {algo_class.__name__} en {instance_name} desde ejecución {completed_runs+1}")
    ...
    try:
        ...
        if parallel and num_processes > 1:
            ...
            for i, task in enumerate(all_tasks):
                logger.info(f"Tarea {i+1}/{len(all_tasks)}: {task[0].__name__} en {task[1]}, run {task[3]}")
                result = _run_algorithm_task(task)
                results_iter.append(result)
                
                # Mostrar progreso
                if (i + 1) % 10 == 0 or i + 1 == len(all_tasks):
                    logger.info(f"Progreso: {i+1}/{len(all_tasks)} ({(i+1)/len(all_tasks)*100:.1f}%)")
        ...
        for result in results_iter:
            ...
            for res in benchmark_results:
                if (res.algorithm_name == result["algorithm"] and 
                    res.instance_name == result["instance"]):
                    target_result = res
                    break
            
            # Si no existe, crear uno nuevo (no debería ocurrir)
            if target_result is None:
                logger.warning(f"Creando resultado nuevo para {result['algorithm']} - {result['instance']}")
                target_result = EnhancedBenchmarkResult(result["algorithm"], result["instance"])
...
def load_results(output_dir, algorithms=None, instances=None):
    ...
    try:
        ...
        for result_data in checkpoint_data.get("results", []):
            ...
            if (algorithms and 
                result_data["algorithm_name"] not in algorithms):
                continue
                
            # Filtrar por instancia si se especifica
            if (instances and 
                result_data["instance_name"] not in instances):
                continue
...
def create_enhanced_report(benchmark_results, output_file=None, include_convergence=True):
    """
    ...
    if include_convergence:
        ...
        for instance_name, results in instance_results.items():
            ...
            for result in results:
                ...
                min_length = min(len(curve) for curve in result.convergence_curves if curve)
                
                if min_length > 0:
                    # Recortar curvas a la misma longitud
                    curves = [curve[:min_length] for curve in result.convergence_curves if curve]
...
def run_complete_analysis(
    ...
    # Generar reporte
    report_file = os.path.join(output_dir, "massive_benchmark_report.html") if output_dir else None
