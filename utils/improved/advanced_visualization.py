...
def create_result_distribution_plot(data, metric='fitness', by='algorithm', output_dir=None):
    """
    ...
    if by == 'algorithm':
        ...
        for instance in instances:
            ...
            if output_dir:
                ...
                fig_path = os.path.join(output_dir, f'distribution_{instance}_{metric}.png')
        ...
        for algorithm in algorithms:
            ...
            if output_dir:
                os.makedirs(output_dir, exist_ok=True)
                fig_path = os.path.join(output_dir, f'distribution_{algorithm}_{metric}.png')
                plt.savefig(fig_path, dpi=300, bbox_inches='tight')
                figures.append(fig_path)
...
def create_convergence_plot(results, instances=None, algorithms=None, confidence_interval=0.95, output_dir=None):
    """
    ...
    if algorithms:
        filtered_results = [r for r in filtered_results if r.algorithm_name in algorithms]
...
def create_cd_diagram(ranks, names, cd=None, output_file=None):
    ...
    # Dibujar posiciones
    ax.scatter(sorted_ranks, np.zeros_like(sorted_ranks), marker='o', s=100, color='black')
...
def create_full_visualization_set(results, data_df, output_dir=None):
    ...
    # CD diagram global
    cd_value = 1.96 * np.sqrt((len(algorithms) * (len(algorithms) + 1)) / (6 * len(instances)))
    figures['cd_diagram'] = create_cd_diagram(
        avg_ranks, algorithms, cd=cd_value,
    ...
    figures['heatmap_mean'] = create_heatmap(
        summary_df, 'Algorithm', 'Instance', 'Mean',
    ...
    figures['heatmap_best'] = create_heatmap(
        summary_df, 'Algorithm', 'Instance', 'Best',
    ...
    figures['heatmap_time'] = create_heatmap(
        summary_df, 'Algorithm', 'Instance', 'Time',
    ...
    radar_df['Mean'] = 1 / radar_df['Mean']  # Invertir para que valores más altos sean mejores
    
    figures['radar'] = create_radar_chart(
        radar_df, instances, 'Algorithm', 'Mean',
        output_file=os.path.join(dirs['radar'], 'radar_chart.png')
    )
    
    logger.info(f"Visualizaciones generadas: {sum(len(f) for f in figures.values())} figuras")
