...
def create_cd_diagram(
    ...
    if cd is not None:
        ...
        for i in range(len(sorted_ranks)):
            for j in range(i + 1, len(sorted_ranks)):
                if abs(sorted_ranks[i] - sorted_ranks[j]) <= cd:
                    ...
                    y_offset = 0.1 * (
                        (j - i) % 5 + 1
                    )  # Vary line height to prevent overlap
                    ax.plot(
                        [i, j],
        ...
        ax.text(
            len(names) / 2,
            2.2,
            f"Critical Difference (CD) = {cd:.2f}",
            ha="center",
            fontsize=12,
...
def generate_stats_report(results, output_file):
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"* Critical Difference: {results['cd_diagram'].split('=')[-1].strip() if 'cd_diagram' in results else 'N/A'}\n\n"
        )
        ...
        if "friedman_p" in results:
            if results["friedman_p"] < 0.05:
                ...
                if "mean_ranks" in results and "critical_distance" in results:
                    ...
                    if len(equivalent) > 1:
                        f.write(
                            f"* Statistically equivalent to the best algorithm ({best_algo}):\n"
                        )
                        for algo in equivalent[1:]:
                            f.write(f"  - {algo}\n")
                    else:
                        f.write(
                            f"* The best algorithm ({best_algo}) is significantly better than all others\n"
                        )
            else:
                f.write(
                    "* No statistically significant differences between algorithms (p >= 0.05)\n"
                )
                f.write(
                    "* All algorithms can be considered equivalent in performance\n"
                )
...
def run_all(csv_path, out_dir):
    ...
    try:
        ...
        if score_col is None:
            logger.error(
                "No se encontró columna de desempeño ('Best' o 'Best Fitness')"
            )
            return {
