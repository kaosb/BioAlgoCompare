...
class VRPOperators:
    ...
    def evaluate_solution(
        routes, distance_matrix, demands, capacity, penalize_infeasible=True
    ):
        """
        ...
        for route in routes:
            total_distance += VRPOperators.calculate_route_distance(
        ...
        while improved and iteration < max_iterations:
            ...
            for i in range(1, len(route) - 2):
                for j in range(i + 1, len(route) - 1):
                    new_route, delta = VRPOperators.two_opt_move(
    ...
    def or_opt_move(route, i, l, j, distance_matrix):
        ...
        new_distance = (
            distance_matrix[new_route[idx_j - 1], new_route[idx_j]]
    ...
    def optimize_all_routes(
        routes, distance_matrix, demands, capacity, max_iterations=50
    ):
        """
        ...
        for route in routes:
            ...
            # Aplicar 2-opt
            improved_route = VRPOperators.two_opt_local_search(
                route, distance_matrix, max_iterations
            )
            ...
            while improved and iteration < max_iterations:
                ...
                for i in range(1, len(current_route) - 1):
                    for j in range(i + 1, len(current_route) - 1):
                        new_route, delta = VRPOperators.swap_nodes(
                            current_route, i, j, distance_matrix
                        )
                ...
                if not improved:
                    for i in range(1, len(current_route) - 1):
                        for j in range(1, len(current_route)):
                            ...
                            new_route, delta = VRPOperators.relocate_node(
                                current_route, i, j, distance_matrix
                            )
                ...
                if not improved:
                    for l in range(
                        2, min(4, len(current_route) - 2)
                    ):  # Longitudes de segmento 2 y 3
                        for i in range(1, len(current_route) - l):
                            for j in range(1, len(current_route) - l + 1):
                                if j >= i and j <= i + l:
                                    continue

                                new_route, delta = VRPOperators.or_opt_move(
                                    current_route, i, l, j, distance_matrix
                                )
            ...
            if VRPOperators.check_route_feasibility(current_route, demands, capacity):
                ...
                optimized_routes.append(
                    route
                )  # Mantener la ruta original si la optimizada no es factible
    ...
    def swap_nodes_between_routes(
        ...
        if not VRPOperators.check_route_feasibility(
            new_route1, demands, capacity
        ) or not VRPOperators.check_route_feasibility(new_route2, demands, capacity):
            return (
                route1,
                route2,
                float("inf"),
            )  # Retornar un delta infinito si no es factible
    ...
    def relocate_between_routes(
        ...
        # Verificar factibilidad
        if (
            len(new_route1) < 3  # Asegurar que route1 no quede vacía
            or not VRPOperators.check_route_feasibility(new_route2, demands, capacity)
        ):
            return (
                route1,
    ...
    @staticmethod
    def cross_exchange(
        route1, route2, i1, j1, i2, j2, distance_matrix, demands, capacity
    ):
        """
        ...
        # Calcular distancia antes del cambio
        old_distance1 = (
            distance_matrix[route1[i1 - 1], route1[i1]]
            + distance_matrix[route1[j1], route1[j1 + 1]]
        )
        old_distance2 = (
            distance_matrix[route2[i2 - 1], route2[i2]]
        ...
        # Calcular distancia después del cambio
        new_distance1 = (
            distance_matrix[new_route1[i1 - 1], new_route1[i1]]
            + distance_matrix[
                new_route1[i1 + len(segment2) - 1], new_route1[i1 + len(segment2)]
            ]
        )
        new_distance2 = (
            distance_matrix[new_route2[i2 - 1], new_route2[i2]]
            + distance_matrix[
                new_route2[i2 + len(segment1) - 1], new_route2[i2 + len(segment1)]
            ]
        )
        ...
        while improved and iteration < max_iterations:
            ...
            for r1 in range(len(current_routes)):
                for r2 in range(r1 + 1, len(current_routes)):
                    ...
                    for i in range(1, len(route1) - 1):
                        for j in range(1, len(route2) - 1):
                            (
                                ...
                                new_route2,
                                delta,
                            ) = VRPOperators.swap_nodes_between_routes(
                                route1, route2, i, j, distance_matrix, demands, capacity
                            )
                    ...
                    for i in range(1, len(route1) - 1):
                        for j in range(1, len(route2)):
                            (
                                ...
                                new_route2,
                                delta,
                            ) = VRPOperators.relocate_between_routes(
                                route1, route2, i, j, distance_matrix, demands, capacity
                            )
                    ...
                    for l1 in range(
                        1, min(4, len(route1) - 2)
                    ):  # Longitudes de segmento limitadas
    ...
    def route_based_crossover(
        parent1_routes, parent2_routes, distance_matrix, demands, capacity
    ):
        """
    ...
    def inversion_mutation(
        routes, distance_matrix, demands, capacity, mutation_rate=0.3
    ):
        """
        ...
        for route in routes:
            ...
            for i, client in enumerate(clients):
                if client > 0 and client <= dimension:  # Ajustar índice si es necesario
                    idx = (
                        client - 1
                    )  # Convertir de índice 1-based a 0-based si es necesario
    ...
    def plot_routes_comparison(
        original_routes, improved_routes, problem, title="Comparación de Rutas"
    ):
        """
        ...
        plt.suptitle(
            f"{title}\nMejora: {(original_distance - improved_distance) / original_distance * 100:.2f}%",
            fontsize=14,
        )
        plt.tight_layout()
    ...
    @staticmethod
    def visualize_route_improvement(
        route, improved_route, problem, title="Mejora de Ruta"
    ):
        """
        ...
        # Calcular distancias
        original_distance = VRPOperators.calculate_route_distance(
            route, problem.distance_matrix
        )
        improved_distance = VRPOperators.calculate_route_distance(
            improved_route, problem.distance_matrix
        )

        # Dibujar ruta original
        for ax, r, dist, subtitle in zip(
            [ax1, ax2],
    ...
    @staticmethod
    def animate_route_optimization(
        routes, improvement_steps, problem, title="Optimización de Rutas", interval=500
    ):
        """
        ...
        def update(frame):
            ...
            current_routes = (
                improvement_steps[frame]
                if frame < len(improvement_steps)
                else improvement_steps[-1]
            )
            ...
            ax.set_title(
                f"Paso {frame+1}/{len(improvement_steps)}\nDistancia Total: {distance:.2f}"
            )
            ax.set_xlabel("Coordenada X")
            ax.set_ylabel("Coordenada Y")
            ax.grid(True, linestyle="--", alpha=0.7)
