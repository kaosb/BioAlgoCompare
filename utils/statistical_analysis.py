...
class StatisticalAnalysis:
    ...
    def friedman_test(data_df, alpha=0.05):
        ...
        if len(data_df['Algorithm'].unique()) < 2:
            return {'error': 'Se requieren al menos 2 algoritmos para la prueba de Friedman'}
        ...
        for instance in instances:
            ...
            for algo in algorithms:
                ...
                values = data_df[(data_df['Instance'] == instance) & 
                                (data_df['Algorithm'] == algo)]['Value'].values
                ...
                if len(values) > 0:
                    ...
                    return {'error': f'Faltan datos para el algoritmo {algo} en la instancia {instance}'}
        ...
        statistic, p_value = friedmanchisquare(*[ranks[:, i] for i in range(ranks.shape[1])])
        ...
        critical_distance = qsturng(1 - alpha, n_algorithms, np.inf) * np.sqrt((n_algorithms * (n_algorithms + 1)) / (6 * n_instances))
    ...
    def kruskal_wallis_test(data_df, alpha=0.05):
        ...
        n_samples = np.mean([len(vals) for vals in algo_values])  # Promedio de muestras por algoritmo
        ...
        critical_distance = qsturng(1 - alpha, n_algorithms, np.inf) * np.sqrt((n_algorithms * (n_algorithms + 1)) / (6 * n_samples))
    ...
    def nemenyi_test(test_result):
        ...
        if 'critical_distance' not in test_result:
            ...
            raise ValueError(f"El resultado de la prueba '{test_name}' no contiene la distancia crítica necesaria para realizar el test post-hoc")
            ...
            for j, algo_j in enumerate(algorithms):
                if i == j:
                    ...
                    if test_type == 'Kruskal-Wallis':
                        ...
                        q = rank_diff / np.sqrt((n_algorithms * (n_algorithms + 1)) / (6 * n_samples))
                    else:
                        # Cálculo original para Friedman
                        q = rank_diff / np.sqrt((n_algorithms * (n_algorithms + 1)) / (6 * n_instances))
    ...
    def wilcoxon_paired_test(data_df, alpha=0.05, bonferroni_correction=True):
        ...
        for algo in algorithms:
            ...
            for instance in instances:
                ...
                values = data_df[(data_df['Algorithm'] == algo) & 
                                (data_df['Instance'] == instance)]['Value'].values
                if len(values) > 0:
        ...
        if bonferroni_correction and len(all_p_values) > 0:
            ...
            reject, adj_p_values, _, _ = multipletests(all_p_values, alpha=alpha, method='bonferroni')
        ...
        try:
            ...
            for algo in algorithms:
                try:
                    ...
                    values = data_df[data_df['Algorithm'] == algo]['Value'].values
                    algo_data[algo] = np.array([float(v) for v in values if not np.isnan(float(v))])
                    ...
                    try:
                        if i == j:
                            ...
                            if len(x) == 0 or len(y) == 0:
                                print(f"Debug - Datos insuficientes para comparar {algo_i} y {algo_j}")
                                a_measure_matrix.loc[algo_i, algo_j] = 0.5
                                continue
                            ...
                            # Calcular A
                            a = p_less / (len(x) * len(y)) if len(x) * len(y) > 0 else 0.5
                            
                            a_measure_matrix.loc[algo_i, algo_j] = a
                    except Exception as e:
                        print(f"Debug - Error al calcular Vargha-Delaney para {algo_i} y {algo_j}: {str(e)}")
                        a_measure_matrix.loc[algo_i, algo_j] = 0.5
    ...
    def plot_critical_difference_diagram(friedman_result, title=None):
        ...
        try:
            ...
            if 'critical_distance' not in friedman_result:
                fig, ax = plt.subplots(figsize=(10, 2))
                ax.text(0.5, 0.5, "No se pudo calcular la distancia crítica para este análisis", 
                        ha='center', va='center')
                ax.axis('off')
                return fig
                
            # Obtener datos necesarios
            algorithms = list(friedman_result['algorithms'])  # Convertir a lista para asegurar indexación
            cd = float(friedman_result['critical_distance'])  # Convertir a float para cálculos
            ...
            for i in range(len(sorted_algos)):
                for j in range(i + 1, len(sorted_algos)):
                    if abs(sorted_ranks[i] - sorted_ranks[j]) <= cd:
                        ...
                        y_pos = -0.05 * (1 + (j - i) * 0.5)  # Ajustar altura para evitar solapamiento
                        ax.plot([sorted_ranks[i], sorted_ranks[j]], [y_pos, y_pos], 'k-', linewidth=2)
            ...
            fig, ax = plt.subplots(figsize=(10, 2))
            ax.text(0.5, 0.5, f"Error al generar diagrama: {str(e)}", ha='center', va='center')
            ax.axis('off')
            return fig
    ...
    def plot_posthoc_heatmap(posthoc_matrix, title=None, alpha=0.05):
        ...
        try:
                ...
                for i in range(len(heatmap_data.index)):
                    for j in range(len(heatmap_data.columns)):
                        try:
                            ...
                            print(f"Debug - Valor no numérico en posición [{i}, {j}]: {heatmap_data.iloc[i, j]}")
                            heatmap_data.iloc[i, j] = 1.0  # Valor neutro para p-values (no significativo)
            except Exception as e:
            ...
            heatmap = sns.heatmap(heatmap_data.astype(float), mask=mask, cmap=cmap, 
                                vmin=0, vmax=1, square=True, linewidths=0.5, 
                                annot=True, fmt='.3f', annot_kws={'size': 9}, ax=ax)
                ...
                for j in range(len(heatmap_data.columns)):
                    if i != j:
                        try:
                            ...
                            print(f"Debug - Error al resaltar celda [{i}, {j}]: {str(e)}")
            ...
            ax.text(0.5, 0.5, f"Error al generar el mapa de calor post-hoc:\n{str(e)}", 
    ...
    def plot_effect_size_heatmap(effect_size_matrix, method='cliff_delta', title=None):
        ...
        try:
                    ...
                    for j in range(len(heatmap_data.columns)):
                        try:
                            ...
                            if method == 'vargha_delaney':
                                heatmap_data.iloc[i, j] = 0.5  # Valor neutro para Vargha-Delaney
                            else:
                                heatmap_data.iloc[i, j] = 0.0  # Valor neutro para otros métodos
            except Exception as e:
            ...
            # Crear el mapa de calor con validación de datos
            heatmap = sns.heatmap(heatmap_data.astype(float), mask=mask, cmap=cmap, 
                                vmin=vmin, vmax=vmax, center=center, square=True, 
                                linewidths=0.5, annot=True, fmt='.3f',
                ...
                for j in range(len(heatmap_data.columns)):
                    if i != j:
                        try:
                            ...
                            interpretation = StatisticalAnalysis.interpret_effect_size(effect, method)
                            text_color = 'white' if abs(effect - center) > (vmax - vmin) * 0.3 else 'black'
                            ax.text(j + 0.5, i + 0.7, interpretation, ha='center', va='center',
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.text(0.5, 0.5, f"Error al generar el mapa de efecto tamaño ({method}):\n{str(e)}", 
                   ha='center', va='center', fontsize=12, wrap=True)
            ax.axis('off')
            plt.tight_layout()
            return fig
    ...
    def plot_rank_boxplot(data_df, friedman_result=None, title=None):
        ...
        sns.stripplot(x='Algorithm', y='Rank', data=ranks_df, color='black', size=4, alpha=0.5, ax=ax)
    ...
    @staticmethod
    def generate_statistical_comparison_table(friedman_result, posthoc_matrix, effect_size_matrix, 
                                             method='cliff_delta', alpha=0.05):
        """
    ...
    @staticmethod
    def generate_statistical_analysis_report(data_df, metric='best_fitness', 
        ...
        if data_df['Instance'].nunique() < 2 and data_df['Algorithm'].nunique() < 2:
            error_msg = f"Datos insuficientes para análisis estadístico: se requieren múltiples instancias o algoritmos."
        ...
        print(f"Debug - Resultado de Friedman:")
        ...
        try:
            ...
            print(f"Debug - Wilcoxon completado con éxito.")
        except Exception as e:
        ...
        try:
            ...
            print(f"Debug - Cálculo de efecto tamaño completado con éxito.")
        except Exception as e:
        ...
        comparison_table, test_info = StatisticalAnalysis.generate_statistical_comparison_table(
            friedman_result, posthoc_matrix, cliff_delta, method='cliff_delta', alpha=alpha)
        ...
        try:
            cd_diagram = StatisticalAnalysis.plot_critical_difference_diagram(
                friedman_result, title=f'Diagrama de Diferencia Crítica - {metric.capitalize()}')
        ...
        try:
            posthoc_heatmap = StatisticalAnalysis.plot_posthoc_heatmap(
                posthoc_matrix, title=f'P-values Post-hoc - {metric.capitalize()}', alpha=alpha)
        ...
        try:
            effect_heatmap = StatisticalAnalysis.plot_effect_size_heatmap(
                cliff_delta, method='cliff_delta', title=f"Cliff's Delta - {metric.capitalize()}")
        ...
        try:
            vd_heatmap = StatisticalAnalysis.plot_effect_size_heatmap(
                vargha_delaney, method='vargha_delaney', title=f"Vargha-Delaney A - {metric.capitalize()}")
        ...
        try:
            rank_boxplot = StatisticalAnalysis.plot_rank_boxplot(
                data_df, friedman_result, title=f'Distribución de Rangos - {metric.capitalize()}')
    ...
    @staticmethod
    def run_comprehensive_statistical_analysis(benchmark_results, metrics=None, 
                                              alpha=0.05, output_dir=None):
        """
        ...
        for metric in metrics:
            try:
                ...
                if len(data_df) > 0:
                    ...
                    if has_nan:
                        print(f"⚠️ Advertencia: La métrica {metric} contiene valores NaN. Se eliminarán antes del análisis.")
                    ...
                    try:
                        ...
                        print(f"⚠️ Error al convertir datos para {metric} a float: {str(e)}")
                        # Intentar limpiar datos problemáticos
                        data_df = data_df[pd.to_numeric(data_df['Value'], errors='coerce').notna()]
                        if len(data_df) == 0:
                            print(f"⚠️ No quedan datos válidos para la métrica {metric} después de la limpieza")
                            continue
                        data_df['Value'] = data_df['Value'].astype(float)
                    
                    # Verificar si hay suficientes datos después de la limpieza
                    if len(data_df) < 10:  # Umbral arbitrario para tener datos significativos
                        print(f"⚠️ Pocos datos ({len(data_df)}) para la métrica {metric} después de la limpieza")
                    
                    # Generar informe
                    output_file = os.path.join(output_dir, f"report_{metric}.html")
                    try:
                        report_path = StatisticalAnalysis.generate_statistical_analysis_report(
                            data_df, metric=metric, alpha=alpha, output_file=output_file)
                        
                        report_paths.append(report_path)
                        successful_metrics.append(metric)
                        print(f"✅ Informe generado para {metric}")
                    except Exception as report_error:
                        print(f"⚠️ Error al generar informe para {metric}: {str(report_error)}")
                else:
