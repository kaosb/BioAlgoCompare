...
class StatisticalReportGenerator:
    ...
    def _run_statistical_tests(self):
        ...
        self.results["friedman"] = self._safe_execute(
            StatisticalAnalysis.friedman_test, self.data_df, alpha=self.alpha
        )
        ...
        if self.results["friedman"] and "reject_h0" in self.results["friedman"]:
            ...
            # Wilcoxon tests
            (
                self.results["wilcoxon"],
                self.results["wilcoxon_effect"],
            ) = self._safe_execute(
    ...
    def _safe_execute(self, func, *args, default=None, **kwargs):
        ...
        try:
            ...
            if "nemenyi" in func.__name__:
                ...
                return pd.DataFrame(
                    1.0, index=algorithms, columns=algorithms
    ...
    def _generate_visualizations(self):
        ...
        viz_methods = {
            ...
            "posthoc_heatmap": (
                ...
                {
                    "title": f"P-values Post-hoc - {self.metric.capitalize()}",
                    "alpha": self.alpha,
                },
            ),
            "effect_heatmap": (
                ...
                {
                    "method": "cliff_delta",
                    "title": f"Cliff's Delta - {self.metric.capitalize()}",
                },
            ),
            "vd_heatmap": (
                StatisticalAnalysis.plot_effect_size_heatmap,
                [self.results.get("vargha_delaney", pd.DataFrame())],
                {
                    ...
                    "title": f"Vargha-Delaney A - {self.metric.capitalize()}",
                },
            ),
        }

        for viz_name, (method, args, kwargs) in viz_methods.items():
            self.visualizations[viz_name] = self._create_visualization(
                method, *args, **kwargs
            )
    ...
    def _create_error_image(self, error_msg):
        ...
        ax.text(
            0.5,
            0.5,
            f"Error al generar gráfico: {error_msg}",
            ha="center",
            va="center",
            fontsize=12,
            color="red",
    ...
    def _prepare_template_data(self):
        ...
        test_info = {
            ...
            if friedman.get("reject_h0", False)
            else "No",
            "Distancia crítica": f"{self.results.get('cd', 0):.4f}"
            if self.results.get("cd")
            else "N/A",
        }
...
"""
    ...
    def _generate_test_results_section(self, data):
        ...
        interpretation = (
            ...
            else f"La prueba {test_info['Prueba']} NO indica diferencias estadísticamente significativas entre los algoritmos comparados (p-value >= {data['alpha']})."
        )
    ...
    def _generate_conclusions_section(self, data):
        ...
        if test_info["Diferencia significativa"] == "Sí" and "rank_dict" in friedman:
            sorted_algorithms = sorted(
                friedman["rank_dict"].items(), key=lambda x: x[1]
            )
            best = sorted_algorithms[0]
            worst = sorted_algorithms[-1]

            conclusions.append(
                f"El algoritmo <strong>{best[0]}</strong> obtuvo el mejor ranking promedio ({best[1]:.2f})."
            )
            conclusions.append(
                f"El algoritmo <strong>{worst[0]}</strong> obtuvo el peor ranking promedio ({worst[1]:.2f})."
            )
            ...
            if "cd" in self.results:
                ...
                similar_to_best = [
                    algo
                    for algo, rank in sorted_algorithms[1:]
                    if abs(rank - best[1]) < cd
                ]
                if similar_to_best:
                    conclusions.append(
                        f"Los algoritmos {', '.join(similar_to_best)} no presentan diferencias significativas con el mejor algoritmo."
                    )
        else:
            conclusions.append(
                "No se encontraron diferencias estadísticamente significativas entre los algoritmos comparados."
            )
