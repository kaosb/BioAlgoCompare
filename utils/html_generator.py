...
def generate_html_report(metric_str, test_info, algo_ranks, friedman_result, posthoc_matrix, cliff_delta, 
                        alpha, cd_img, rank_img, posthoc_img, effect_img, vd_img):
    """
    ...
    if test_info['Diferencia significativa'] == 'Sí':
        html.append(f'La prueba {test_info["Prueba"]} indica que hay diferencias estadísticamente significativas entre los algoritmos comparados (p-value < {alpha}).')
    else:
        html.append(f'La prueba {test_info["Prueba"]} NO indica diferencias estadísticamente significativas entre los algoritmos comparados (p-value >= {alpha}).')
    html.append('</p>')
    html.append('</div>')
    ...
    html.append('<p>Simbología: + (mejor), - (peor), = (no hay diferencia significativa)</p>')
    html.append('<table>')
    html.append('<tr>')
    html.append('<th>Algoritmo</th>')
    ...
    html.append(f'<img src="data:image/png;base64,{cd_img}" alt="Critical Difference Diagram">')
    ...
    html.append('<div class="caption">Figura 2: Distribución de rangos por algoritmo</div>')
    html.append('</div>')
    
    html.append('<div class="figure">')
    html.append(f'<img src="data:image/png;base64,{posthoc_img}" alt="Post-hoc Test p-values">')
    html.append('<div class="caption">Figura 3: Matriz de p-values de prueba post-hoc</div>')
    html.append('</div>')
    
    html.append('<div class="figure">')
    html.append(f'<img src="data:image/png;base64,{effect_img}" alt="Effect Size - Cliff\'s Delta">')
    html.append('<div class="caption">Figura 4: Tamaño del efecto (Cliff\'s Delta)</div>')
    html.append('</div>')
    
    html.append('<div class="figure">')
    html.append(f'<img src="data:image/png;base64,{vd_img}" alt="Effect Size - Vargha-Delaney A">')
    html.append('<div class="caption">Figura 5: Tamaño del efecto (Vargha-Delaney A)</div>')
    html.append('</div>')
    ...
    html.append(f'<p>Este análisis estadístico para la métrica <strong>{metric_str}</strong> ')
    
    if test_info['Diferencia significativa'] == 'Sí':
        html.append('muestra diferencias estadísticamente significativas entre los algoritmos.')
    else:
        html.append('no muestra diferencias estadísticamente significativas entre los algoritmos.')
    ...
    if test_info['Diferencia significativa'] == 'Sí' and algo_ranks:
        ...
        html.append(f'<li>El algoritmo <strong>{best_algo}</strong> obtuvo el mejor ranking promedio ({algo_ranks[0][1]:.2f}).</li>')
        html.append(f'<li>El algoritmo <strong>{worst_algo}</strong> obtuvo el peor ranking promedio ({algo_ranks[-1][1]:.2f}).</li>')
        ...
        if not_diff_from_best:
            html.append(f'<li>Los algoritmos {", ".join(not_diff_from_best)} no son significativamente diferentes del mejor algoritmo ({best_algo}).</li>')
    else:
        html.append('<li>No se encontraron diferencias estadísticamente significativas entre los algoritmos comparados.</li>')
