...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Ejecuta benchmark extendido en instancias Solomon VRP"
    )
    parser.add_argument(
        "--runs", type=int, default=50, help="Número de ejecuciones (default: 50)"
    )
    parser.add_argument(
        "--iterations",
        type=int,
    ...
    try:
        ...
        if not benchmark_dir:
            raise ValueError(
                "No se pudo extraer el directorio de resultados del benchmark"
            )
        ...
        with open(md_file, "w") as f:
            ...
            if summary_data:
                f.write(
                    "| Algoritmo | Mejor Fitness | Fitness Promedio | Desviación Std | Tiempo (s) |\n"
                )
                f.write(
                    "|-----------|--------------|------------------|----------------|------------|\n"
                )
                ...
                for row in summary_data:
                    f.write(
                        f"| {row['Algorithm']} | {float(row['Best']):.2f} | {float(row['Mean']):.2f} | "
                    )
                    f.write(f"{float(row['Std']):.2f} | {float(row['Time']):.4f} |\n")

            f.write("\n## Gráficos\n\n")
            f.write(
                f"Los gráficos detallados se encuentran en el directorio `{analysis_dir}`:\n\n"
            )
            ...
            if summary_data:
                ...
                f.write(
                    f"- El algoritmo **{best_algo}** obtuvo los mejores resultados (menor distancia).\n"
                )
                f.write(f"- El algoritmo **{fastest_algo}** fue el más rápido.\n")
                f.write(
                    f"- El algoritmo **{most_stable}** mostró mayor estabilidad (menor desviación estándar).\n\n"
                )

            f.write(
                f"Para más detalles, consulte el reporte HTML completo: `{output_dir}/massive_benchmark_report.html`\n"
            )
