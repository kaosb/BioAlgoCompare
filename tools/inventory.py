...
def get_imports(file_path):
    ...
    try:
        ...
        try:
            ...
            import_pattern = (
                r"(?:from\s+(\w+(?:\.\w+)*)\s+import|\bimport\s+(\w+(?:\.\w+)*))"
            )
    ...
    for root, dirs, files in os.walk(repo_root):
        ...
        for file in files:
            ...
            if any(file.endswith(ext) for ext in PY_EXTENSIONS):
                ...
                local_imports = [
                    imp for imp in imports if is_local_import(imp, module_list)
                ]
                ...
                for imp in local_imports:
                    ...
                    if len(parts) > 1:
                        ...
                        if os.path.exists(
                            os.path.join(repo_root, parts[0], "__init__.py")
                        ):
...
def detect_data_usage(files_info):
    ...
    data_files = [
        ...
        and any(f["path"].endswith(ext) for ext in DATA_EXTENSIONS)
    ]
    ...
    for file_info in files_info:
        if any(file_info["path"].endswith(ext) for ext in PY_EXTENSIONS):
            ...
            try:
                with open(
                    os.path.join(os.getcwd(), file_info["path"]), "r", encoding="utf-8"
                ) as f:
                    content = f.read()
...
def generate_inventory_report(files_info, imports_map, module_list, data_usages):
    ...
    imported_modules = sorted(
        [(k, v) for k, v in imports_map.items()], key=lambda x: len(x[1]), reverse=True
    )
    ...
    orphaned_files = [
        ...
        and not f["path"].startswith(
            "algorithms/"
        )  # Los algoritmos son el núcleo del sistema
        and not f["path"] == "problems/vrp.py"
    ]  # El problema VRP es central
    ...
    data_usage_sorted = sorted(
        [(k, v) for k, v in data_usages.items()], key=lambda x: len(x[1]), reverse=True
    )
