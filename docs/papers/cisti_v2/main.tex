...
% Metodología
\section{Metodología}\label{sec:metodologia}\vspace{-0.2cm}

\subsection{Sistema Experimental}

Implementamos los dieciséis algoritmos en Python 3.11.5, asegurando un marco comparativo justo con interfaces comunes, representóción unificada y funciones de evaluación idénticas. Las implementaciones siguieron estrictamente las publicaciones originales, adaptando los operadores para el dominio discreto del VRP cuando fue necesario.
...
% Resultados
\section{Resultados}\vspace{-0.2cm}
...
Para ilustrar el rendimiento de los algoritmos en distintos tipos de instancias, la Tabla~\ref{tab:fitness} presentó el fitness promedio (distancia total) con intervalos de confianza del 95\% obtenido por los cinco mejores algoritmos en cada instancia.

% Tabla fitness por instancia (5 mejores algoritmos)
\begin{table}[ht]
  \centering
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.95\columnwidth]{figures/tiempo_por_iteracion.png}
...
Para analizar la relación entre coste computacional y calidad de solución, la Fig.~\ref{fig:cost_quality} mostró un diagrama de dispersión donde el eje x (escala logarítmica) representó el tiempo promedio por iteración y el eje y el fitness promedio.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.95\columnwidth]{figures/scatter_cost_quality.png}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.95\columnwidth]{figures/cd_diagram.png}
...
\begin{table}[htbp]
  \centering
  \caption{Ejemplo de p-valores (prueba de Holm) para algoritmos seleccionados}
  \label{tab:pvalues}
  \small
...
La Tabla~\ref{tab:pvalues} presentó un extracto de los p-valores de la prueba de Holm para algoritmos seleccionados. Los valores indican la probabilidad de que dos algoritmos tengan rendimiento similar; valores p < 0.05 (en negrita) señalan diferencias estadísticamente significativas.

Complementando la significancia estadística, calculamos los tamaños de efecto A12 de Vargha-Delaney \cite{Vargha2000} para cuantificar la magnitud práctica de las diferencias. La Tabla~\ref{tab:a12} mostró estos valores para algoritmos seleccionados.
...
\begin{itemize}
\item WOA mostró un efecto grande (A12 > 0.71) cuando se compara con algoritmos fuera del grupo élite.
\item Entre WOA, MRFO y SMA, los valores A12 están próximos a 0.5, confirmando rendimiento estadísticamente equivalente.
\item AHA presentó valores A12 = 0 frente a los algoritmos de alto rendimiento, indicando inferioridad sistemática.
\end{itemize}

% Discusión
\section{Discusión}\vspace{-0.2cm}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.95\columnwidth]{figures/algoritmos_por_serie.png}
  \caption{Comparación de fitness por tipo de instancia Solomon.}
  \label{fig:series}
\end{figure}

La Fig.~\ref{fig:series} revela otro patrón interesante: los algoritmos mostraron comportamiento diferenciado entre las series 101 y 201. Algoritmos como WOA y FSA parecen adaptarse mejor a las restricciones temporales estrechas de las series 201.
...
\section{Amenazas a la validez}\vspace{-0.2cm}
...
% Conclusiones
\section{Conclusiones y Trabajo Futuro}\vspace{-0.2cm}
...
\section{Agradecimientos}\vspace{-0.2cm}
...
\section{Apéndice de Reproducibilidad}\vspace{-0.2cm}
...
% Aseguramos el balance de columnas en la última página
\flushend
