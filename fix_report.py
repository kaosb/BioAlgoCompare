#\!/usr/bin/env python3
"""
Script para corregir el archivo statistical_analysis.py
"""

import os

# Definición de la nueva implementación
        ...
        try:
            ...
            try:
                vd_heatmap = StatisticalAnalysis.plot_effect_size_heatmap(
                    vargha_delaney, method='vargha_delaney', title=f"Vargha-Delaney A - {metric_str}")
            ...
            html_parts.append(f'<p>Generado: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>')
            ...
            if test_info['Diferencia significativa'] == 'Sí':
                html_parts.append(f'La prueba {test_info["Prueba"]} indica que hay diferencias estadísticamente significativas entre los algoritmos comparados (p-value < {alpha}).')
            else:
                html_parts.append(f'La prueba {test_info["Prueba"]} NO indica diferencias estadísticamente significativas entre los algoritmos comparados (p-value >= {alpha}).')
            html_parts.append('</p>')
            ...
            if not 'error' in friedman_result and algo_ranks:
                html_parts.append('<div class="section">')
                html_parts.append('<h2>Comparaciones entre Algoritmos</h2>')
                html_parts.append('<p>Simbología: + (mejor), - (peor), = (no hay diferencia significativa)</p>')
                html_parts.append('<table>')
                html_parts.append('<tr>')
                html_parts.append('<th>Algoritmo</th>')
                    ...
                    for j, (algo_j, _) in enumerate(algo_ranks):
                        if i == j:
                            ...
                            try:
                                ...
                                html_parts.append(f'<td>{symbol} (p={p_value:.3f}, d={effect:.3f})</td>')
                            except Exception as e:
            ...
            html_parts.append(f'<img src="data:image/png;base64,{cd_img}" alt="Critical Difference Diagram">')
            html_parts.append('<div class="caption">Figura 1: Diagrama de diferencia crítica</div>')
            html_parts.append('</div>')
            
            html_parts.append('<div class="figure">')
            html_parts.append(f'<img src="data:image/png;base64,{rank_img}" alt="Rank Distribution">')
            html_parts.append('<div class="caption">Figura 2: Distribución de rangos por algoritmo</div>')
            html_parts.append('</div>')
            
            html_parts.append('<div class="figure">')
            html_parts.append(f'<img src="data:image/png;base64,{posthoc_img}" alt="Post-hoc Test p-values">')
            html_parts.append('<div class="caption">Figura 3: Matriz de p-values de prueba post-hoc</div>')
            html_parts.append('</div>')
            
            html_parts.append('<div class="figure">')
            html_parts.append(f'<img src="data:image/png;base64,{effect_img}" alt="Effect Size - Cliff\'s Delta">')
            html_parts.append('<div class="caption">Figura 4: Tamaño del efecto (Cliff\'s Delta)</div>')
            html_parts.append('</div>')
            
            html_parts.append('<div class="figure">')
            html_parts.append(f'<img src="data:image/png;base64,{vd_img}" alt="Effect Size - Vargha-Delaney A">')
            html_parts.append('<div class="caption">Figura 5: Tamaño del efecto (Vargha-Delaney A)</div>')
            html_parts.append('</div>')
            ...
            html_parts.append(f'<p>Este análisis estadístico para la métrica <strong>{metric_str}</strong> ')
            
            if test_info['Diferencia significativa'] == 'Sí':
                html_parts.append('muestra diferencias estadísticamente significativas entre los algoritmos.')
            else:
                html_parts.append('no muestra diferencias estadísticamente significativas entre los algoritmos.')
            ...
            if not 'error' in friedman_result and test_info['Diferencia significativa'] == 'Sí' and algo_ranks:
                best_algo = algo_ranks[0][0]
                worst_algo = algo_ranks[-1][0]
                
                html_parts.append(f'<li>El algoritmo <strong>{best_algo}</strong> obtuvo el mejor ranking promedio ({algo_ranks[0][1]:.2f}).</li>')
                html_parts.append(f'<li>El algoritmo <strong>{worst_algo}</strong> obtuvo el peor ranking promedio ({algo_ranks[-1][1]:.2f}).</li>')
                ...
                if not_diff_from_best:
                    html_parts.append(f'<li>Los algoritmos {", ".join(not_diff_from_best)} no son significativamente diferentes del mejor algoritmo ({best_algo}).</li>')
            else:
                html_parts.append('<li>No se encontraron diferencias estadísticamente significativas entre los algoritmos comparados.</li>')
            ...
            # Guardar el informe - usando join con newline para crear el contenido HTML
            with open(output_file, 'w') as f:
                f.write('\\n'.join(html_parts))
            ...
            error_file = output_file or f"results/error_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
...
file_path = "utils/statistical_analysis.py"
with open(file_path, 'r') as f:
    content = f.read()

# Encontrar el método a reemplazar
...
# Encontrar el fin del método
rest_content = content[start_idx:]
next_method = rest_content.find('@staticmethod', 10)  # Buscar después de la primera aparición
...
if next_method == -1:
    ...
    # Es un método intermedio
    end_idx = start_idx + next_method
...
# Guardar el archivo con el método reemplazado
with open(file_path, 'w') as f:
    f.write(new_content)

print(f"Se ha reemplazado el método 'generate_statistical_analysis_report' en {file_path}")
