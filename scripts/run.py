...
def run_algorithm(algo_name, problem, population, iterations, run_seed, run_id):
    try:
        ...
        return {"algorithm": algo_name.upper(), "run": run_id, "error": str(e)}
...
def main(
    ...
    if seed is not None:
        ...
        random.seed(seed)
        np.random.seed(seed)
        click.echo(f"Semilla global establecida: {seed}")

    # Determinar número de procesos para paralelización
    num_processes = (
        min(mp.cpu_count(), runs * len(algorithms_to_run)) if parallel else 1
    )
    ...
    for algo_name in algorithms_to_run:
        for run in range(1, runs + 1):
            ...
            all_tasks.append(
                (algo_name, problem, population, iterations, run_seed, run)
            )
    ...
    if parallel and num_processes > 1:
        ...
        try:
            with Pool(processes=num_processes) as pool:
                ...
                for result in tqdm(
                    pool.imap_unordered(run_algo_wrapper, all_tasks),
                    total=len(all_tasks),
                ):
                    if "error" in result:
                        click.echo(
                            f"Error en {result['algorithm']}, ejecución {result['run']}: {result['error']}"
                        )
        ...
        for algo_name, problem, population, iterations, run_seed, run in all_tasks:
            ...
            if "error" in result:
                ...
                click.echo(
                    f"  Mejor fitness = {result['best_fitness']:.2f}, Tiempo = {result['execution_time']:.2f}s"
                )
    ...
    if visualize:
        ...
        for algo, indices in algo_results.items():
            ...
            # Visualizar solución
            # Manejar algoritmos que devuelven posición en formato de rutas
            if isinstance(best_solution.position, list) and isinstance(
                ...
                # Calcular la distancia total (sin penalizaciones)
                total_distance = 0
                for route in routes:
                    for i in range(len(route) - 1):
                        total_distance += problem.distance_matrix[
                            route[i], route[i + 1]
                        ]
            else:
                # Convertir representación continua a rutas
                routes, total_distance, _ = problem.decode_solution(
                    best_solution.position
                )

            plt = plot_vrp_solution(
                problem,
                routes,
                f"{algo} - {instance} - Distancia: {total_distance:.2f}",
            )
            ...
            plt = plot_convergence(
                results["convergence"][best_idx], f"{algo} - Curva de Convergencia"
            )
    ...
    if len(algorithms_to_run) > 1 and visualize:
        ...
        plt = compare_algorithms(
            avg_convergence, f"Comparación de Algoritmos - {instance}"
        )
    ...
    if save:
        ...
        if parallel:
            ...
            click.echo(
                f"Información de paralelización guardada en results/{instance}_{timestamp}_parallel_info.json"
            )
