...
Parameterized by PAIR (default the screen's top balanced pair GWO+GA). Robustness
sweep: re-run on other balanced complementary pairs (DE+GA, PSO+ACO) and at 50D to
check that the pattern holds — the gate DETECTS adversarial transfer (CTRL+) while
positive transfer does NOT beat the portfolio (C2p≈C1_div, C3p≈ABL).

Conditions (all same total budget B, assert fes_used == B):
  C0_A / C0_B : each solver isolated, full B                (references)
  C1_div      : both isolated at B/2, no comms, min          (portfolio floor)
  C2p         : parametric transfer, gate OFF
  C3p         : parametric + gate ON (proposed)
  ABL         : parametric, gate ON, RANDOM intent           (cross-solver attribution)
  CTRLn/CTRLg : ADVERSARIAL intent, gate OFF / ON            (positive control)

    RUNS=31 python scripts/cec_harness/run_parametric_ladder.py            # GWO+GA 10D
    PAIR=DE,GA python .../run_parametric_ladder.py                         # other pair
    PAIR=PSO,ACO SBS=ACO python .../run_parametric_ladder.py
    PAIR=GWO,GA DIM=50 python .../run_parametric_ladder.py                 # 50D

Output: results/cooperative/parametric_ladder_<A><B>_<DIM>d.json (+ provenance).
Fresh seeds 4000+. F22 excluded (anomalous).
"""
...
# a-priori single-best-solver per pair (mean rank from complementarity screen)
_SBS_DEFAULT = {"GWO+GA": "GWO", "DE+GA": "GA", "PSO+ACO": "ACO"}
...
POP = 30
TRANSFER_EVERY = 500
PAIR = os.environ.get("PAIR", "GWO,GA").split(",")
A_NAME, B_NAME = PAIR[0].strip(), PAIR[1].strip()
SBS = os.environ.get("SBS", _SBS_DEFAULT.get(f"{A_NAME}+{B_NAME}", A_NAME))
SPECS = [(A_NAME, _CLS[A_NAME], {"population_size": POP, "max_iterations": 10**9}),
         (B_NAME, _CLS[B_NAME], {"population_size": POP, "max_iterations": 10**9})]

OUT_DIR = os.path.join(_REPO_ROOT, "results", "cooperative")
OUT_PATH = os.path.join(OUT_DIR,
                        f"parametric_ladder_{A_NAME}{B_NAME}_{DIM}d.json")
...
def _coop(fnum, seed, gate, intent_mode):
    ...
    runner = CooperativeRunner(prob, SPECS, transfer_every=TRANSFER_EVERY,
                               ...
                               transfer_mode="parametric", intent_mode=intent_mode)
    res = runner.run()
...
def main():
    ...
    ca, cb = f"C0_{A_NAME}", f"C0_{B_NAME}"
    raw, gate_stats = {}, {}
    print(f"[param-ladder] {A_NAME}+{B_NAME} {DIM}D (SBS={SBS}), "
          f"{len(FUNCS)} funcs x {RUNS} runs, B={BUDGET}")
    for f in FUNCS:
        cols = {ca: [], cb: [], "C1_div": [], "C2p": [], "C3p": [],
                "ABL": [], "CTRLn": [], "CTRLg": []}
        ...
        for r in range(RUNS):
            s = 4000 + r
            cols[ca].append(_isolated(_CLS[A_NAME], f, s, BUDGET))
            cols[cb].append(_isolated(_CLS[B_NAME], f, s, BUDGET))
            cols["C1_div"].append(min(_isolated(_CLS[A_NAME], f, s, BUDGET // 2),
                                      _isolated(_CLS[B_NAME], f, s, BUDGET // 2)))
        ...
        print(f"  F{f:<3d} {A_NAME}={m[ca]:.2e} {B_NAME}={m[cb]:.2e} "
    ...
    out = {"config": {"pair": f"{A_NAME}+{B_NAME}", "sbs": SBS, "funcs": FUNCS,
                      "dim": DIM, "budget": BUDGET, "runs": RUNS, "pop": POP,
                      "transfer_every": TRANSFER_EVERY,
                      "note": "fresh seeds 4000+; F22 excluded"},
           "raw_medians": {f: {c: float(np.median(v)) for c, v in cols.items()}
                           for f, cols in raw.items()},
           "gate_stats": gate_stats, "contrasts": results,
           "provenance": provenance()}
    with open(OUT_PATH, "w") as fh:
        json.dump(out, fh, indent=2)

    print(f"\n=== CONTRASTES {A_NAME}+{B_NAME} {DIM}D (A mejor que B; Wilcoxon+Holm) ===")
