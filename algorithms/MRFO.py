...
class MantaRay(Individual):
    ...
    def move(self, best_ray, t, T, alpha=2.0):
        ...
        beta = 2 * math.exp(1 - (t / T)) * math.sin(2 * math.pi * r)  # Controla la espiral
        
        if t / T < 0.5:  # Primera mitad: forrajeo en cadena
            for i in range(self.dimension):
                r1 = random.random()
                # Seguir a la mejor mantarraya con factor de espiral
                self.position[i] = best_ray.position[i] + beta * (best_ray.position[i] - self.position[i]) + alpha * r1
        else:  # Segunda mitad: forrajeo en ciclón
            for i in range(self.dimension):
                r2 = random.random()
                if random.random() < 0.5:  # Espiral externa (alejándose)
                    # Comportamiento de ciclón
                    self.position[i] = best_ray.position[i] + alpha * math.exp(beta * r2) * math.cos(2 * math.pi * r2) * abs(best_ray.position[i] - self.position[i])
                else:  # Espiral interna (acercándose)
                    # Exploración de profundidad
                    self.position[i] = best_ray.position[i] + random.random() * (best_ray.position[i] - self.position[i]) + beta * r2
        ...
        if random.random() < somersault_prob:
            for i in range(self.dimension):
                self.position[i] += random.uniform(-1, 1) * abs(best_ray.position[i] - self.position[i])
    ...
    def update_population(self):
        ...
        for i in range(self.population_size):
            ...
            if self.population[i] is not self.best_solution:
                ...
                self.population[i].move(self.best_solution, current_iter, self.max_iterations, self.spiral_factor)
