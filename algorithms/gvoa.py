...
class Vulture(Individual):
    ...
    def move(self, leader_pos, informed_positions=None, phase='following', r=0.5, it=0, max_it=100):
        """
        ...
        if phase == 'following':
            ...
            new_position = self.position + decay * r1 * (leader_pos - self.position) + r * r2 * (np.random.random(self.dimension) - 0.5)

        elif phase == 'foraging' and informed_positions is not None and len(informed_positions) > 0:
            ...
            new_position = self.position + r1 * (target_pos - self.position) + decay * r2 * (leader_pos - self.position)
            ...
            if random.random() < 0.5:
                ...
                new_position[idx] = np.clip(new_position[idx] + a * (random.random() - 0.5), 0, 1)
            else:
                # Perturbación aleatoria en todas las dimensiones
                new_position = self.position + a * (np.random.random(self.dimension) - 0.5)
            ...
            if random.random() < 0.5:
                ...
                new_position = leader_pos + radius * (np.random.random(self.dimension) - 0.5)
            else:
                # Exploración alejada del líder
                new_position = leader_pos + (1 + radius) * (np.random.random(self.dimension) - 0.5)
    ...
    def update_population(self):
        ...
        for i, vulture in enumerate(self.population):
            ...
            if frac < 0.3:
                ...
                phase = 'scouting' if i >= int(self.population_size * frac) else 'search'
            else:
                # later: more exploitation
                phase = 'foraging' if i < int(self.population_size * (1 - frac)) else 'following'
