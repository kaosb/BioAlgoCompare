"""
Tests para verificar la migración de AHA a la nueva arquitectura v2.
"""
...
from algorithms.aha import AHA, Hummingbird
from algorithms.aha_v2 import AHAV2, HummingbirdV2
from problems.vrp import VRPProblem


class TestAHAV2Migration:
    """Tests para la migración de AHA a v2."""
    ...
    def test_initialization_compatibility(self, test_problem):
        ...
        v1 = AHA(test_problem, population_size=pop_size, 
                                         max_iterations=max_iter, seed=seed)
        v2 = AHAV2(test_problem, population_size=pop_size, 
                                           max_iterations=max_iter, seed=seed)
    ...
    def test_individual_creation(self, test_problem):
        ...
        dim = test_problem.get_dimension()
        bounds = [(0, 1) for _ in range(dim)]
        position = np.random.uniform(0, 1, size=dim)
        ind_v1 = Hummingbird(position, bounds)
        ind_v2 = HummingbirdV2(test_problem)
        ind_v2.initialize()
    ...
    def test_reproducibility(self, test_problem):
        ...
        v2_1 = AHAV2(test_problem, population_size=pop_size, 
                                             max_iterations=max_iter, seed=seed)
        best_v2_1 = v2_1.execute()
        
        v2_2 = AHAV2(test_problem, population_size=pop_size, 
                                             max_iterations=max_iter, seed=seed)
