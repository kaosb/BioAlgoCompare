...
class TestTiming(unittest.TestCase):
    ...
    def test_calculate_avg_summary_with_data(self, mock_iteration_times):
        ...
        algo1_entry = next(
            (entry for entry in summary if entry["algorithm"] == "algo1"), None
        )
        algo2_entry = next(
            (entry for entry in summary if entry["algorithm"] == "algo2"), None
        )
        ...
        self.assertAlmostEqual(
            algo1_entry["avg_iter_time"], 0.15, places=10
        )  # (0.1 + 0.2) / 2
    ...
    def test_instrumentation(self):
        ...
        mock_original = MagicMock(
            return_value={"algorithm": "test_algo", "instance": "test_inst"}
        )
