...
def test_vrp_distance_matrix():
    ...
    for i in range(problem.dimension):
        for j in range(i+1, problem.dimension):
            assert np.isclose(problem.distance_matrix[i, j], problem.distance_matrix[j, i])
...
def test_decode_solution():
    ...
    assert covered_nodes == required_nodes, f"Missing nodes: {required_nodes - covered_nodes}"
...
def test_feasibility_check():
    ...
    assert problem.routes_are_feasible(routes) == True
    
    # Crear una ruta infactible (excede capacidad)
    infeasible_route = [[problem.depot_index] + list(range(1, problem.dimension)) + [problem.depot_index]]
    assert problem.routes_are_feasible(infeasible_route) == False
    
    # Crear una ruta con cliente faltante
    missing_client_routes = [[problem.depot_index, 1, problem.depot_index], 
                            [problem.depot_index, 3, 4, 5, problem.depot_index]]
    assert problem.routes_are_feasible(missing_client_routes) == False
...
def test_repair_mechanism():
    ...
    assert problem.routes_are_feasible(repaired_routes) == True
