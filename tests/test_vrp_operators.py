...
def test_evaluate_solution(vrp_problem, sample_routes):
    ...
    total_distance, is_feasible = VRPOperators.evaluate_solution(
        sample_routes,
        vrp_problem.distance_matrix,
        vrp_problem.demands,
        vrp_problem.capacity,
    )
...
def test_swap_nodes(vrp_problem, sample_route):
    ...
    new_route, delta = VRPOperators.swap_nodes(
        sample_route, i, j, vrp_problem.distance_matrix
    )
    ...
    old_distance = VRPOperators.calculate_route_distance(
        sample_route, vrp_problem.distance_matrix
    )
    new_distance = VRPOperators.calculate_route_distance(
        new_route, vrp_problem.distance_matrix
    )
...
def test_two_opt_move(vrp_problem):
    ...
    new_route, delta = VRPOperators.two_opt_move(
        route, i, j, vrp_problem.distance_matrix
    )
    ...
    # Verificar que el delta de distancia es correcto
    old_distance = VRPOperators.calculate_route_distance(
        route, vrp_problem.distance_matrix
    )
    new_distance = VRPOperators.calculate_route_distance(
        new_route, vrp_problem.distance_matrix
    )
...
def test_optimize_all_routes(vrp_problem, sample_routes):
    ...
    for route in optimized_routes:
        assert VRPOperators.check_route_feasibility(
            route, vrp_problem.demands, vrp_problem.capacity
        )
...
def test_swap_nodes_between_routes(vrp_problem):
    ...
    pytest.skip(
        "Este test necesita revisión para considerar restricciones de capacidad"
    )
