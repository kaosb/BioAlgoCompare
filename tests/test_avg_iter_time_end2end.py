...
def test_avg_iter_time_in_summary_csv():
    ...
    os.environ['PYTHONPATH'] = f"{python_path}:{os.getcwd()}" if python_path else os.getcwd()
    ...
    assert "avg_iter_time" in df.columns, "La columna avg_iter_time no está presente en el CSV"
    
    # Verificar que el valor es mayor que 0
    assert df["avg_iter_time"].iloc[0] > 0, "El tiempo promedio por iteración debe ser mayor que 0"
