...
def test_cli_invalid_instance(runner):
    ...
    result = runner.invoke(main, ['--algorithm', 'opa', '--instance', 'nonexistent_instance'])
    assert result.exit_code == 0  # Click maneja el error y no devuelve un código de error
    assert "Error: La instancia nonexistent_instance no existe en data/vrp" in result.output

def test_cli_invalid_algorithm(runner):
    """Test que verifica el comportamiento con un algoritmo inválido."""
    result = runner.invoke(main, ['--algorithm', 'invalid_algo', '--instance', 'P-n16-k8'])
...
def test_cli_parameter_validation(runner):
    ...
    result = runner.invoke(main, ['--algorithm', 'opa', '--instance', 'P-n16-k8', '--iterations', 'abc'])
    assert result.exit_code != 0
