"""
Tests para verificar la migración de OPA a la nueva arquitectura v2.
"""
...
from algorithms.opa_v2 import OPAV2, OrcaV2
from problems.vrp import VRPProblem


class TestOPAV2Migration:
    """Tests para la migración de OPA a v2."""
    ...
    def test_initialization_compatibility(self, test_problem):
        ...
        v1 = OPA(test_problem, population_size=pop_size, 
                                         max_iterations=max_iter, seed=seed)
        v2 = OPAV2(test_problem, population_size=pop_size, 
                                           max_iterations=max_iter, seed=seed)
    ...
    def test_individual_creation(self, test_problem):
        ...
        # Crear individuo v2 (v1 usa representación diferente)
        ind_v2 = OrcaV2(test_problem)
        ind_v2.initialize()
        
        # Verificar propiedades básicas de v2
        assert ind_v2.dimension == test_problem.get_dimension()
        assert len(ind_v2.position) == ind_v2.dimension
        assert ind_v2.position.shape == (ind_v2.dimension,)
        
        # Verificar que las posiciones estén en límites válidos
        assert np.all(ind_v2.position >= 0) and np.all(ind_v2.position <= 1)
        
        # Nota: OPA v1 usa representación basada en rutas, no continua
    ...
    def test_reproducibility(self, test_problem):
        ...
        v2_1 = OPAV2(test_problem, population_size=pop_size, 
                                             max_iterations=max_iter, seed=seed)
        best_v2_1 = v2_1.execute()
        
        v2_2 = OPAV2(test_problem, population_size=pop_size, 
                                             max_iterations=max_iter, seed=seed)
