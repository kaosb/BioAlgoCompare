...
def test_statistical_analysis_smoke(tmp_path):
    ...
    ), f"Error in statistical analysis: {results.get('error', 'Unknown error')}"

    # Check that the output files exist
    assert os.path.exists(
        os.path.join(out_dir, "cd_diagram.png")
    ), "CD diagram not created"
    assert os.path.exists(
        os.path.join(out_dir, "stats_report.md")
    ), "Stats report not created"
    ...
    assert (
        results["friedman_p"] < 0.05
    ), "Expected significant p-value but got nonsignificant"
    ...
    if not results["a12"].empty:
        ...
        assert (
            a12_value1vs2 > 0.5
        ), f"Expected A12 > 0.5 for algo1 vs algo2 but got {a12_value1vs2}"
        assert (
            a12_value1vs3 > 0.5
        ), f"Expected A12 > 0.5 for algo1 vs algo3 but got {a12_value1vs3}"
        assert (
            a12_value2vs3 > 0.5
        ), f"Expected A12 > 0.5 for algo2 vs algo3 but got {a12_value2vs3}"
