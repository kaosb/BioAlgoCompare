...
class TestIndividualInterface:
    ...
    def test_individual_inheritance(self, algo_name):
        ...
        module = __import__(
            f"algorithms.{algo_name}", fromlist=[individual_classes[algo_name]]
        )
    ...
    def test_individual_has_required_methods(self, algo_name, vrp_problem):
        ...
        assert hasattr(
            individual, "fitness"
        ), f"{algo_name} Individual missing fitness method"
        assert hasattr(
            individual, "is_better_than"
        ), f"{algo_name} Individual missing is_better_than method"
        assert hasattr(
            individual, "is_feasible"
        ), f"{algo_name} Individual missing is_feasible method"
        assert hasattr(
            individual, "move"
        ), f"{algo_name} Individual missing move method"
        assert hasattr(
            individual, "copy"
        ), f"{algo_name} Individual missing copy method"
    ...
    def test_is_better_than_method(self, algo_name, vrp_problem):
        ...
        if result:
            assert not ind2.is_better_than(
                ind1
...
class TestAlgorithmInterface:
    ...
    def test_algorithm_inheritance(self, algo_name):
        ...
        AlgoClass = ALGORITHMS[algo_name]
        assert issubclass(
            AlgoClass, MetaheuristicAlgorithm
    ...
    def test_algorithm_required_methods(self, algo_name, vrp_problem):
        ...
        assert hasattr(algo, "run"), f"{algo_name} missing run method"
        assert hasattr(
            algo, "get_best_solution"
    ...
    def test_initialize_population(self, algo_name, vrp_problem):
        ...
        assert len(algo.population) == 10, f"{algo_name} population size mismatch"

        # Check best solution is tracked
        assert hasattr(algo, "best_solution"), f"{algo_name} should track best_solution"
        assert (
            algo.best_solution is not None
    ...
    def test_update_population(self, algo_name, vrp_problem):
        ...
        assert hasattr(
            algo, "convergence_curve"
        ), f"{algo_name} should have convergence_curve"
        assert (
            len(algo.convergence_curve) >= 2
    ...
    def test_get_convergence_curve(self, algo_name, vrp_problem):
        ...
        assert len(curve) == 4, f"{algo_name} convergence curve length mismatch"
        assert all(
            isinstance(v, (int, float)) for v in curve
